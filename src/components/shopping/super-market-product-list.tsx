...
export default function SuperMarketProductList() {
  ...
  const onDeleteEvent = (product: Product) =>
    dispatch(shoppingSlice.actions.delete(product));
  const onEditEvent = (product: Product) =>
    dispatch(
      shoppingSlice.actions.changeStatus({
        ...product,
        status: ProductStatus.EDITION,
      }),
    );
